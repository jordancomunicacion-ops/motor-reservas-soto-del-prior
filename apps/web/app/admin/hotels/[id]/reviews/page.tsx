...
export default function HotelReviewsPage() {
    ...
    useEffect(() => {
        fetchAPI<ReviewsResponse>(`/restaurant/hotel/${hotelId}/reviews`)
            .then(setData)
    ...
    return (
        <div className="space-y-8 max-w-5xl mx-auto pb-20">
            ...
            {data.total === 0 && (
                <div className="bg-amber-50 dark:bg-amber-900/10 border border-amber-100 dark:border-amber-900/30 p-4 rounded-lg text-sm text-amber-800 dark:text-amber-200">
                    Si este hotel no tiene un restaurante vinculado, las valoraciones se gestionan desde la ficha del restaurante. Vincula la sinergia hotel-restaurante en la configuración del restaurante.
                </div>
            )}
            ...
            <Card>
                ...
                <CardContent>
                    {data.items.length === 0 ? (
                        <p className="text-sm italic text-muted-foreground py-8 text-center">Aún no hay valoraciones.</p>
                    ) : (
